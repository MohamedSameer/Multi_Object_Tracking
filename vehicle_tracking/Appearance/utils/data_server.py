...
import threading
import time
import Appearance.utils.get_datasets as get_datasets
...
class BatchCacheServer:
    ...
    def add_dataset(self, dataset_name):
        ...
        for xx in range(gt.shape[0]):
            ...
            # Add the key.
            # key = [dataset_ind, video_id, main_img_seq, pos_img_seq, main_track_id, neg_track_id]
            self.all_keys.add((dataset_ind, start_line[0], start_line[1], start_line[2], start_line[3], start_line[4]))
            num_keys += 1
    ...
    def lookup_func(self, key):
        ...
        try:
            ...
            next_ind = key[3]
            if self.debug:
                prev_imageName = self.image_paths[key[0]][prev_ind]
                next_imageName = self.image_paths[key[0]][next_ind]
                print('Reading prev_image', prev_imageName)
                print('Reading next_image', next_imageName)

            prev_path = self.image_paths[key[0]][prev_ind]
            #prev_image = cv2.imread(prev_path)[:, :, ::-1]
            prev_image = cv2.imread(prev_path)
            prev_shape = pickle.dumps(prev_image.shape)
            prev_string = prev_image.tostring()
            images.append((prev_string, prev_shape))

            next_path = self.image_paths[key[0]][next_ind]
            #next_image = cv2.imread(next_path)[:, :, ::-1]
            next_image = cv2.imread(next_path)
            next_shape = pickle.dumps(next_image.shape)
            next_string = next_image.tostring()
            images.append((next_string, next_shape))
...
if __name__ == '__main__':
    ...
    #parser.add_argument('-n', '--num_unrolls', action='store', default=2, dest='num_unrolls', type=int)
