...
def training(inputs, num_unrolls, mem_size, batch_size, labels, debug=False, train=False, reuse=None, feature_size=256):
    ...
    if debug:
        return predicted_appearance_feature, predicted_bbox, image_label_feature, appearance_feature
    else:
...
def RAN(appearance_external_memory, motion_external_memory, batch_size, num_unrolls, mem_size, prev_lstm_state1=None, prev_lstm_state2=None, reuse=None, debug=False):
    ...
    with tf.variable_scope('RAN', reuse=reuse):
        ...
        #latest_appearance_feature = appearance_external_memory[:, :, 0]
        #latest_motion_feature = motion_external_memory[:, :, 0]

        # Update LSTM
        swap_memory = num_unrolls > 1
        with tf.variable_scope('appearance_lstm'):
            # Initialize LSTM cell
            appearance_lstm = tf.contrib.rnn.LSTMCell(256 * cell_ratio, use_peepholes=True, initializer=msra_initializer, reuse=reuse)
        ...
        with tf.variable_scope('motion_lstm'):
            ...
            motion_lstm = tf.contrib.rnn.LSTMCell(4 * mem_size * 2, use_peepholes=True, initializer=msra_initializer, reuse=reuse)
        ...
        with tf.variable_scope('fc_appearance'):
            # predicted_appearance_feature = [batch * num_unrolls, feature_size]
            predicted_appearance_feature = tf.layers.dense(appearance_output, feature_size, activation=tf.nn.softmax, use_bias=True,
                                  bias_initializer=tf.zeros_initializer(), reuse=reuse)

        with tf.variable_scope('fc_motion'):
            # predicted_bbox = [batch * num_unrolls, 4]
            predicted_bbox = tf.layers.dense(motion_output, 4, activation=tf.nn.sigmoid,
...
def loss(outputs, labels):
    ...
    #motion_loss     = tf.reduce_mean(tf.reduce_sum(tf.sqrt(tf.abs(motion_label - predicted_motion)), axis=1), axis=0)
    motion_loss     = tf.reduce_sum(tf.reduce_sum(tf.abs(predicted_motion - motion_label), axis=1))
